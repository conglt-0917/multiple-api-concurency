...
// boundedParallelPost sends requests in parallel but only up to a certain
// limit, and furthermore it's only parallel up to the amount of CPUs but
// is always concurrent up to the concurrency limit
func boundedParallelPost(urls []string, concurrencyLimit int) []result {
	requestBody, _ := json.Marshal(map[string]string {
		"creator_id": "1",
		"family_tree_id": "2",
		"receiver_ids" : "3",
		"title": "Create concurency",
		"message": "Golang",
		"condition": "go",
		"item_ids": "123",
	})
...
func main() {
	benchmark := func(urls []string, concurrency int) string {
		startTime := time.Now()
		results := boundedParallelPost(urls, concurrency)
